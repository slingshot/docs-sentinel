...
# Tracked modifications the auditor made. It cannot create files (Write is disallowed), so there
# are no untracked files to consider. (while-read, not mapfile: bash 3.2 portability.)
CHANGED=()
...
nfiles=${#CHANGED[@]}
nlines=$(git diff --numstat | awk '{a+=$1; d+=$2} END {print a+d+0}')
